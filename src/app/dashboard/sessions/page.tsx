...
import type { Session } from "../../../store/api/sessionApi";
...
export default function Sessions() {
  ...
  // Handle sorting and filtering in useEffect to avoid hydration issues
  useEffect(() => {
    if (!mounted || !sessionsData?.sessions) return;

    const filtered = [...sessionsData.sessions]
      .filter((session) =>
        session.title.toLowerCase().includes(searchQuery.toLowerCase())
      )
      .sort((a, b) => {
        if (sortBy === "recent") {
          // Use ISO string comparison for consistent sorting
          return b.lastAccessed.localeCompare(a.lastAccessed);
        }
        return b.progress - a.progress;
      });

    setSortedSessions(filtered);
  }, [mounted, sessionsData?.sessions, searchQuery, sortBy]);

  // Show nothing during initial mount to avoid hydration issues
  if (!mounted) {
    return null;
  }
